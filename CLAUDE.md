...
5. **`docs/obo/` と `docs/superpowers/` はローカル専用**（.gitignore 済み）。コミットしないこと。
   ...
   **ガイドの整形ルール**: 段落・文の途中で改行しない（1 段落 = 1 行。
   80 桁前後での折り返しを入れない）。
6. **ドキュメントは日本語がマスター。多言語を同時更新する。** `README.md` /
   `docs/0X-*.md` を新規作成・編集するときは必ず `docs-sync` スキルに従い、日本語を先に
   書いてから対応する翻訳（`docs/i18n/<lang>/`）を**同一コミット内で**追従させ、
   `scripts/check-doc-sync.sh --update` でマニフェストを更新する。CI（`doc-sync.yml`）が
   追従漏れを検知して fail させる。
