...
`/wiring-check` にトークンなし、および実トークンではないダミー文字列でアクセスし、obo プラグインが 401 を返すことを確認します。いずれもプラグインがリクエストを拒否して upstream には到達しないため、echo の応答が表示される場面はありません（ローカル echo は Route の転送先を成立させるために置いています）。upstream は検証専用のローカル echo であり token B の正規 audience ではないため、この Route には実トークンを送らないでください。
